...
vignette: >
  %\VignetteIndexEntry{Advanced Dimensionality Reduction}
...
This vignette covers advanced non-linear dimensionality reduction methods available in tidyul:

- **ISOMAP** (Isometric Feature Mapping): Preserves geodesic distances along manifolds
- **Self-Organizing Maps (SOM)**: Neural network-based topological mapping
- **Autoencoders**: Deep learning approach for dimensionality reduction

These methods are particularly useful when data has complex, non-linear structure that linear methods like PCA cannot capture.
...
- You want to preserve geodesic (along-manifold) distances

**Use SOM when:**
- You want both dimensionality reduction and clustering
- Preserving topological relationships is important
- You need interpretable, visualizable representations

**Use Autoencoders when:**
- Data has highly complex, non-linear patterns
- You have sufficient data for training
- You want flexible, customizable architectures

## ISOMAP (Isometric Feature Mapping)

ISOMAP extends MDS by computing distances along a neighborhood graph, effectively "unfolding" non-linear manifolds.

### Basic ISOMAP

```{r isomap-basic, eval=FALSE}
# Prepare iris data
...
### Visualizing ISOMAP Results

```{r isomap-plot, eval=FALSE}
# Basic plot
plot(isomap_result)
...
SOMs create a low-dimensional (typically 2D) representation while preserving topological properties of the input space.

### Basic SOM

```{r som-basic, eval=FALSE}
...
```{r autoencoder-basic, eval=FALSE}
...
```{r autoencoder-training, eval=FALSE}
...
```{r autoencoder-quality, eval=FALSE}
...
|--------|----------|------|------|
| **ISOMAP** | Manifold data | Preserves geodesic distances | Requires tuning k |
| **SOM** | Topological mapping | Interpretable grid | Less precise embedding |
| **Autoencoders** | Complex patterns | Highly flexible | Needs more data, longer training |

## Practical Tips

### 1. Data Scaling

Always scale data before applying these methods:

```{r scaling-tip, eval=FALSE}
...
**ISOMAP**:
- Use `tidy_isomap()` with appropriate k value
- Check stress for quality assessment
- Best for manifold-structured data

**Self-Organizing Maps**:
- Use `tidy_som()` with grid_dim and rlen parameters
- Provides both dimensionality reduction and topological mapping
- Multiple visualization types available

**Autoencoders**:
- Use `tidy_autoencoder()` with customizable architecture
- Monitor reconstruction error
- Most flexible but requires more tuning

All methods complement linear techniques (PCA, MDS) and can reveal patterns that linear methods miss.

For basic dimensionality reduction, see the "PCA and MDS" vignette. For clustering applications, see the "Clustering Methods" vignette.
